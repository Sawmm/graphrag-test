...
- **System name and version:** MedGemma 4B (instruction-tuned, `-it`); base model google/medgemma-4b-pt; Gemma 3 variant; Decoder-only Transformer architecture; Input modalities text and vision; Output modality text only; context length at least 128K tokens; Model version 1.0.1; Model created July 9, 2025.
...
MedGemma was evaluated across a range of different multimodal classification, report generation, visual question answering, and text-based tasks. MedGemma models have been evaluated on a comprehensive set of clinically relevant benchmarks, including over 22 datasets across 5 different tasks and 6 medical image modalities, with a focus on expert human evaluations for tasks like CXR report generation and radiology VQA.
